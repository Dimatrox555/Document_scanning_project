...
#TODO пофиксить (пересмотреть) классичесике паттерны 
patterns ={
   "pattern_lib" : {
        ...
        #TODO пофиксить тему с сканированием О и 0 
        "doc_num_classic":r"[0О]\s+ПОВЫШЕНИИ\s+КВАЛИФИКАЦИИ\s+(\d+)",
        "doc_num_t2":r"\d{12}",
        "date_from_classic": r"с\s+(\d{2}\s+[а-яё]+\s+\d{4}\s+г\.)\sпо\s+\d{2}\s+[а-яё]+\s+\d{4}\s+г\.",
        "date_to_classic": r"с\s+\d{2}\s+[а-яё]+\s+\d{4}\s+г\.\sпо\s+(\d{2}\s+[а-яё]+\s+\d{4}\s+г\.)",
        "city_name_classic":r"Город\s([А-ЯЁ][а-яё]+(?:-[А-ЯЁ]?[а-яё]+)*)",
        "new_spec_classic": r"программе\s+«([^»]+)»",
        "hours_num_classic": r"объёме\s+(\d+)",
    ...
    "doc_schemas" : {
        "doc_type1": {
            ...
            "date_from" : ["date_from_classic"],
            "date_to" : ["date_to_classic"],
